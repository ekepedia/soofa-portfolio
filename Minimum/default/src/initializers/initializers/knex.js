...
MySQLService.init(function (err, knex) {
    ...
    knex.schema.hasTable('associations').then(function(exists) {
        if (!exists) {
            return knex.schema.createTableIfNotExists("associations", function (table) {
                MySQLService.define_associations_schema(table);
            }).catch(function (error) {
                winston.error(error);
            });
        }
    ...
    knex.schema.hasTable('association_requests').then(function(exists) {
        if (!exists) {
            return knex.schema.createTableIfNotExists("association_requests", function (table) {
                MySQLService.define_association_request_schema(table);
            }).catch(function (error) {
                winston.error(error);
            });
        }
