...
function deliver_welcome_videos(user_id) {
    ...
    UserService.getOne(user_id, function (err, user) {
        ...
        winston.info("Active time for: " + user.user_id + " is: " + active_time);

        var messages = _.map(videos, function (original_video) {

            var video = clone(original_video);
        ...
        _.each(messages, function (message, index) {
            ...
            var welcome_message = MySQLService.convert_to_firebase_object(message_id, message);

            MySQLService.push_message(welcome_message, function (err) {
                if(err) {
                    winston.error("Error when trying to send welcome videos: " + err.sqlMessage);
                    winston.error("Error when trying to send welcome videos: " + JSON.stringify(welcome_message, null, 4));
                } else {
                    winston.info("message $" + welcome_message.key + " added to SQL database");
                }
            });

            /*var story_message_id = "welcome-story-" + index + "@" + user_id;

            message.recipient_id = "story-@" + user_id;

            var story_message = MySQLService.convert_to_firebase_object(story_message_id, message);

            MySQLService.push_message(story_message, function (err) {
                if(err) {
                    winston.error("Error when trying to send welcome videos: " + err.sqlMessage);
                    winston.error("Error when trying to send welcome videos: " + JSON.stringify(story_message, null, 4));
                } else {
                    winston.info("message $" + story_message.key + " added to SQL database");
                }
            }); */
        });
    });
